...
# ── Register Deriv Flask routes if Flask is available ────────────────
if FLASK_OK:
    try:
...
if __name__ == "__main__":
    ...
    if render_env and FLASK_OK:
        ...
        if _DERIV_TOKEN:
            mode = "DEMO" if _DERIV_DEMO else "REAL"
            log_info(f"[Deriv] Token configured - {mode} account")
            log_info("[Deriv] DerivBroker will connect on first scan cycle")
        else:
            log_warn("[Deriv] DERIV_API_TOKEN not set - Deriv disabled")
...
class DerivWSClient:
    ...
    def recv(self) -> Optional[Dict]:
        """Receive one JSON response. Blocks up to 15s."""
...
class DerivBroker(BrokerBase):
    ...
    def __init__(self, token: str = "", demo: bool = True):
        ...
        self._ws_url    = _DERIV_WS_DEMO if self._demo else _DERIV_WS_REAL
        self._ws        = DerivWSClient(self._ws_url)
        self._authed    = False
        self._balance   = 0.0
        self._currency  = "USD"
        self._lock      = _dthread.Lock()
        self._req_id    = 1
        self._positions: Dict[str, Dict] = {}   # contract_id -> position
    ...
    def _ensure_connected(self) -> bool:
        """Connect and authorize if not already done."""
        if self._ws.connected and self._authed:
            return True
        if not self._token:
            return False
    ...
    def _authorize(self) -> bool:
        """Send authorize request with API token."""
        resp = self._ws.send_recv({
            "authorize": self._token,
            "req_id": self._next_id(),
        }, timeout=10)

        if resp and "error" not in resp and "authorize" in resp:
            info             = resp["authorize"]
            self._balance    = float(info.get("balance", 0))
            self._currency   = info.get("currency", "USD")
            self._authed     = True
            mode = "DEMO" if self._demo else "REAL"
            log_info(f"[Deriv] Authorized ({mode}) "
