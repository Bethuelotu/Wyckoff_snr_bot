...
def run():
    ...
    while True:
        try:
            # ── Internet gate — check BEFORE every scan ───────────
            if needs_internet:
                if not check_internet():
                    log_warn("Internet lost — pausing bot until reconnected...")
                    require_internet(retry_secs=30)
                    log_info("Internet restored — resuming scans.")

            scan_count += 1
            log_info(f"--- Scan #{scan_count} started ---")
            ...
            for symbol in CONFIG["symbols"]:
                ...
                try:
                    ...
                    if sig:
                        log_signal(
                            f"{sig.direction.value} {symbol} | "
                            f"Score:{sig.score} (W:{sig.score_w}+S:{sig.score_s}) | "
                            f"Pattern:{sig.pattern.value} | "
                        ...
                        if not can_trade:
                            ...
                            # All three layers aligned — execute
                            tick  = broker.get_tick(symbol)
                            entry = float(tick["ask"]) \
                                    if sig.direction == Direction.BUY \
                                    else float(tick["bid"]) \
                                    if tick else sig.snr_price
                            lot   = calc_lot_size(broker, sig.sl, entry)
                            result = broker.place_order(
                                symbol, sig.direction, lot, sig.sl, sig.tp
                            )
                            if result:
                                msg = (
                                    f"TRADE: {sig.direction.value} {symbol}\n"
                                    f"Score:{sig.score} | {sig.pattern.value}\n"
                                    f"Bias:{bias_dir}@{bias_tf} | "
                                    f"Entry trigger:{entry_analysis['trigger']}\n"
                                    f"Entry:{entry:.5f} | SL:{sig.sl:.5f} | "
                                    f"TP:{sig.tp:.5f} | Lot:{lot}"
                                )
                                log_trade(msg)
                                send_telegram(msg)
                                can_trade = (len(broker.get_open_positions())
                                             < CONFIG["max_trades"])
                    else:
                    ...
                    # Internet dropped mid-scan — pause immediately, do not continue
