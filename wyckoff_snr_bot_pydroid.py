...
# ── Deriv signal handoff queue ─────────────────────────────────────────
# The Yahoo scan loop pushes signals here instead of waiting on Deriv
# execution directly. A separate worker thread (run_deriv_worker) drains
# this queue independently, so a slow/hanging Deriv proposal never blocks
# the next symbol scan.
DERIV_SIGNAL_QUEUE: "_dqueue.Queue" = _dqueue.Queue()
...
class YFinanceBroker(BrokerBase):
    ...
    SYM_MAP = {
        ...
        "USD_CAD": "CADUSD=X",  "USD_CHF": "CHFUSD=X",
        "AUD_USD": "AUDUSD=X",  "NZD_USD": "NZDUSD=X",
        "EUR_JPY": "EURJPY=X",  "EUR_GBP": "EURGBP=X",
    }
...
class YFinanceMT5Broker(YFinanceBroker):
    ...
    def __init__(self):
        ...
        self._deriv = None   # lazy-init, only created if a synthetic is requested

    def _get_deriv(self) -> "DerivBroker":
        if self._deriv is None:
            self._deriv = DerivBroker()
        return self._deriv

    def get_candles(self, symbol: str, timeframe: str,
                    count: int = 200) -> Optional[pd.DataFrame]:
        """
        Synthetics  -> Deriv (Yahoo doesn't carry them at all)
        Crypto      -> Binance (real volume)
        Everything else (forex, metals, indices) -> Yahoo Finance
        """
        if symbol in DERIV_SYNTHETIC_MAP:
            df = self._get_deriv().get_candles(symbol, timeframe, count)
            if df is not None:
                return df
            log_warn(f"Deriv candles failed for {symbol} {timeframe}")
            return None   # no Yahoo fallback possible for synthetics
...
def _tg_cmd_status(chat_id: str) -> None:
    try:
        deriv_state = "🟢 CONNECTED" if CONFIG.get("deriv_token") or \
                      os.environ.get("DERIV_API_TOKEN") else "⚫ NO TOKEN"
...
API_KEY: str = os.environ.get("API_KEY", "your-secret-key")
...
def run_session_bot_loop() -> None:
    ...
    while True:
        try:
            ...
            push_all_data(push_data)
            # ── Hand off to Deriv worker thread (non-blocking) ────────
            # The Deriv worker thread (run_deriv_worker) picks these up
            # independently and runs them through run_deriv_scan_v2 /
            # run_deriv_scan, so a slow Deriv proposal never blocks the
            # next Yahoo scan cycle.
            if os.environ.get("DERIV_API_TOKEN", ""):
                for sig in cycle_data.get("signals", []):
                    DERIV_SIGNAL_QUEUE.put(sig)
            # ─────────────────────────────────────────────────────────
...
class DerivWSClient:
    """
    WebSocket client for Pydroid 3 (pure stdlib, Python 3.8).

    Router architecture
    ───────────────────
    A single background reader thread owns the socket exclusively.
    It continuously drains incoming frames and routes each one to the
    caller that is waiting for it, keyed on req_id.

    Each send_recv() call:
      1. Registers a slot: {event: threading.Event, result: None}
      2. Sends the JSON frame (send_lock serialises writes)
      3. Waits on its Event (up to `timeout` seconds)
      4. Returns the result that the reader thread deposited

    This eliminates all race conditions where concurrent callers would
    steal each other's frames from the socket.

    Push frames (msg_type without req_id, e.g. tick streams) are
    delivered to an optional push_callback if registered.
    """
    ...
    def __init__(self, url: str):
        ...
        # One lock serialises all socket writes
        self._send_lock  = _dthread.Lock()

        # Router table: req_id -> {"event": Event, "result": Optional[Dict]}
        self._pending: Dict[int, Dict] = {}
        self._router_lock = _dthread.Lock()

        # Optional callback for unsolicited push frames (tick streams etc.)
        self._push_callback = None  # callable(Dict) or None
    ...
    def connect(self) -> bool:
        """Open WebSocket connection and start the reader thread."""
        try:
            import re
            ...
            self._sock.settimeout(5.0)   # short poll interval so partial frame reads don't stall the reader
            ...
            self._stop_reader = False
            self._reader_thread = _dthread.Thread(
    ...
    def set_push_callback(self, cb):
        """Register a callable(Dict) for unsolicited push frames."""
        self._push_callback = cb

    # ── Frame encoding ───────────────────────────────────────────────

    def _encode_frame(self, payload: str) -> bytes:
        """Encode a masked text frame (RFC 6455 §5.3)."""
    ...
    # ── Frame decoding (called only from reader thread) ──────────────

    def _recv_exact(self, n: int) -> bytes:
        buf = b""
        while len(buf) < n:
            if self._stop_reader or not self._connected:
    ...
    def _read_one_frame(self) -> Optional[str]:
        ...
        Returns the text payload or None (ping/close/error).
        ...
        if opcode == 8:   # close
            ...
            reason = ""
            if length >= 2:
                code_bytes = self._recv_exact(min(length, 125))
                code = _struct.unpack("!H", code_bytes[:2])[0]
                reason = code_bytes[2:].decode("utf-8", errors="replace")
                log_warn(f"[Deriv WS] Server close: code={code} reason={reason}")
            else:
                log_warn("[Deriv WS] Server closed connection")
            return None
    ...
    def _reader_loop(self):
        """
        Background thread: owns the socket for reading.
        Routes every incoming frame to the waiting send_recv() caller,
        or hands it to the push callback for unsolicited frames.
        """
        log_info("[Deriv WS] Reader thread running")
        while self._connected and not self._stop_reader:
            try:
                raw = self._read_one_frame()
                if raw is None:
                ...
                try:
                    ...
                    log_warn(f"[Deriv WS] Non-JSON frame ignored: {raw[:200]}")
                    continue
                ...
                if not routed:
                    ...
                    if self._push_callback:
                        try:
                            ...
                            log_error(f"[Deriv WS] Push callback error: {cb_err}")
                    else:
                        log_info(f"[Deriv WS] Unrouted frame "
                                 f"req_id={req_id} msg_type={msg_type} "
                                 f"(no push callback registered)")
    ...
    def send(self, payload: Dict) -> bool:
        ...
        try:
            with self._send_lock:
                self._sock.sendall(self._encode_frame(json.dumps(payload)))
            return True
        except Exception as e:
    ...
    def send_recv(self, payload: Dict, timeout: float = 10.0) -> Optional[Dict]:
        ...
        The req_id in `payload` is the routing key.  The background
        reader thread deposits the response into this slot and fires
        the event; send_recv() wakes up and returns it.

        Thread-safe: multiple callers can call this concurrently.
        """
        req_id = payload.get("req_id")
        if req_id is None:
            log_error("[Deriv WS] send_recv called without req_id")
            return None

        # Register slot BEFORE sending to avoid a race where the
        # response arrives before we have registered
        ...
        try:
            ...
            if not fired:
                log_warn(f"[Deriv WS] send_recv timeout "
                         f"req_id={req_id} after {timeout}s")
                return None

            result = slot["result"]
            if result is None:
                # Reader woke us because it is shutting down
                log_warn(f"[Deriv WS] Reader shutdown while waiting "
                         f"req_id={req_id}")
            return result
    ...
    def recv(self) -> Optional[Dict]:
        """
        Non-blocking poll — returns the next push frame if one is
        available within 0.1 s, otherwise None.
        Convenience wrapper; prefer set_push_callback for streaming.
        """
        # This is a lightweight shim; real push delivery uses the callback.
        # We just peek at the socket state for health checks.
        return None   # push frames go to callback, not here
...
class DerivBroker(BrokerBase):
    ...
    def _on_push_frame(self, msg: Dict):
        """
        Receive unsolicited push frames from the router's background thread.
        ...
        if msg_type == "proposal_open_contract":
            ...
            if cid and cid in self._positions:
                # Update PnL and status in the cached position
                with self._lock:
                    self._positions[cid]["pnl"]    = float(poc.get("profit", 0))
                    self._positions[cid]["status"]  = poc.get("status", "open")
                    self._positions[cid]["bid"]     = float(poc.get("bid_price", 0))
                log_info(f"[Deriv Push] Contract {cid} pnl="
                         f"{self._positions[cid]['pnl']:.4f} "
                         f"status={self._positions[cid]['status']}")

        # Tick stream update — data used by TradeWatcher, suppress log noise
        elif msg_type == "tick":
            pass
    ...
    def _ensure_connected(self) -> bool:
        ...
        # Register push callback so unsolicited frames (proposal_open_contract
        # updates, tick streams) are captured rather than dropped
        self._ws.set_push_callback(self._on_push_frame)
...
_deriv_broker_lock: "_dthread.Lock"           = _dthread.Lock()
...
def _get_deriv_broker() -> Optional[DerivBroker]:
    """Return singleton DerivBroker, creating if needed. Thread-safe."""
    global _deriv_broker
    _token = os.environ.get("DERIV_API_TOKEN", "")
    if not _token:
        return None
    if _deriv_broker is None:
        with _deriv_broker_lock:
            if _deriv_broker is None:   # re-check after acquiring lock
            ...
            try:
                ...
                for cid in list(self._watch_list.keys()):
                    if cid not in live_ids:
                        ...
                        symbol = pos.get("symbol", "")
                        # Determine win/loss from last known PnL in cached position
                        last_pnl = float(pos.get("pnl", -DERIV_CONFIG["stop_loss_usd"]))
                        won      = last_pnl > 0
                        self._acct.record_trade_result(
                            won=won,
                            pnl=last_pnl
                        )
                        log_info(f"[TradeWatcher] {cid} result: "
                                 f"{'WIN' if won else 'LOSS'} pnl={last_pnl:.4f}")
...
def run_deriv_worker() -> None:
    """
    Background thread: pulls signals off DERIV_SIGNAL_QUEUE and runs them
    through the existing run_deriv_scan_v2 / run_deriv_scan pipeline,
    independently of the Yahoo scan loop (run_session_bot_loop).

    This means a slow or hanging Deriv proposal/order never delays the
    next symbol scan - the two loops run fully in parallel.
    """
    log_info("[Deriv] Worker thread started")
    while True:
        try:
            sig = DERIV_SIGNAL_QUEUE.get(timeout=5)
        except _dqueue.Empty:
            continue
        batch = [sig]
        # Drain any other signals already queued from the same cycle
        # so they're evaluated together (matches original batch behaviour).
        while not DERIV_SIGNAL_QUEUE.empty():
            try:
                batch.append(DERIV_SIGNAL_QUEUE.get_nowait())
            except _dqueue.Empty:
                break
        try:
            if callable(globals().get("run_deriv_scan_v2")):
                run_deriv_scan_v2(batch)
            elif callable(globals().get("run_deriv_scan")):
                run_deriv_scan(batch)
            if callable(globals().get("deriv_sync_positions")):
                deriv_sync_positions()
        except Exception as e:
            log_error(f"[Deriv] Worker scan error: {e}")
...
if __name__ == "__main__":
    render_env = os.environ.get("RENDER") or os.environ.get("PORT")
    ...
    if render_env and FLASK_OK:
        ...
        if os.environ.get("DERIV_API_TOKEN", ""):
            deriv_thread = threading.Thread(
                target=run_deriv_worker, name="DerivWorker", daemon=True
            )
            deriv_thread.start()
            log_info("[Deriv] Worker thread started — runs independently of scan loop")
